...
<style>
  ...
  [data-testid="metric-container"] label { color: #a0a8c0 !important; font-size: 0.8rem !important; }
  ...
  h2 { color: #c4b5fd !important; font-weight: 600 !important; }
  h3 { color: #93c5fd !important; font-weight: 500 !important; }
  p, li, span, label { color: #d1d5db !important; }
  ...
  .stTabs [data-baseweb="tab"] {
    color: #9ca3af !important; border-radius: 8px;
  ...
  /* Dataframes */
  .dataframe { background: rgba(255,255,255,0.04) !important; border-radius: 10px; }

  /* Info boxes */
  .stAlert { border-radius: 12px; border: none !important; }

  /* Select boxes & sliders */
  .stSelectbox > div > div, .stSlider { color: #e0e0e0 !important; }
  ...
  .pred-churn {
    ...
    border: 2px solid rgba(239,68,68,0.5); border-radius: 16px;
    padding: 1.5rem; text-align: center;
  }
  .pred-no-churn {
    background: linear-gradient(135deg, rgba(52,211,153,0.2), rgba(16,185,129,0.1));
    border: 2px solid rgba(52,211,153,0.5); border-radius: 16px;
    padding: 1.5rem; text-align: center;
  }
</style>
...
@st.cache_resource(show_spinner="Training models… (first run only, grab a ☕)")
...
if page == "🏠 Overview":
    ...
    <div class="hero-banner">
      ...
      <p style="color:#9ca3af !important; font-size:0.9rem">
        Powered by Random Forest · Built from CustomerChurn Notebook
      </p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col2:
        st.metric("📉 Churned (%)",     f"{churn_pct}%")
    with col3:
        st.metric("🔢 Features",        "24")
    with col4:
        st.metric("🎯 Target",          "churn_risk_score")

    st.markdown("---")
    st.subheader("📌 Project Pipeline")
    cols = st.columns(5)
    steps = [
        ("1️⃣ Data Ingestion",   "Load churn.csv (36,992 rows)"),
        ("2️⃣ Cleaning",         "Handle nulls, dirty placeholders, type errors"),
        ("3️⃣ Feature Engg.",    "Date decomposition, referral, visit period"),
        ("4️⃣ Modeling",         "6 classifiers compared via F1 & ROC-AUC"),
        ("5️⃣ Deployment",       "Live Streamlit predictor"),
    ]
    ...
    schema = {
        ...
        "Notes":  ["18-65","M/F","City/Town/Village","Platinum/Premium/…","Join date",
                   ...
                   "Minutes","₹","Days","Points","Yes/No","Yes/No","Yes/No",
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    if submitted:
        ...
        try:
            ...
            with col_b:
                if pred == 1:
                    ...
                    <div class="pred-churn">
                      <h2 style="color:#f87171 !important; margin:0">⚠️ High Churn Risk</h2>
                    ...
                    <div class="pred-no-churn">
                      <h2 style="color:#34d399 !important; margin:0">✅ Low Churn Risk</h2>
            ...
            st.warning("Make sure the model is trained. Navigate to '🤖 Model Comparison' first.")
